...
const FileInput = ({name , label , value , placeholder , readOnly = false}) => {
    ...
    return ( 
        <div className={`flex ring-2 rounded-sm text-lg text-slate-800 overflow-hidden transition-all duration-150 ${focus ? 'ring-palete-4-500-1' : 'ring-palete-2-400-1'}`}>
            ...
            <input onFocus={handleFocus} onBlur={handleFocus} name={name} type='file' className="appearance-none w-5/6 py-2 px-2 text-xl focus:outline-none cursor-pointer"/>
        </div>
    );
}
