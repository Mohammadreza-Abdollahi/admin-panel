...
const ProductPaginationTable = ({data , dataInfo , currentPage , pageCount , setPageCount , setCurrentPage , rowInPage , searchable = false , dialogOpenner , searchParam = 'title' , hasBtn = true , hasBackBtn = false}) => {
                ...
                <table className="text-center w-full bg-palete-2-100 bg-opacity-60 rounded-sm overflow-hidden ring-1 ring-palete-2-300">
                    <thead className="border-b-palete-2-300 border-b-4 bg-palete-2-200 bg-opacity-70">
                        <tr className="text-slate-800">
                            {
                                dataInfo.map(i=> (
                                    <th key={i.field ? i.field : 'no_field'} className="py-3">{i.title}</th>
                                ))
                            }
                        </tr>
                    </thead>
                                ...
                                <tr key={d.id} className="transition-all duration-150 border-b-palete-2-200 border-b-2 hover:bg-palete-2-300 hover:bg-opacity-50">
                                    {
                                        dataInfo.map(i=>(
                                            i.field ? (
                                                <td key={`${i.field}${d.id}`} className={`ring-1 ring-palete-2-200`}>{typeof d[i.field] == 'number' ? (d[i.field].toLocaleString('fa')) : (d[i.field])}</td>
                                            ) : (
                                                <td key={`${i.field}${d.id}`} className={`ring-1 ring-palete-2-200`}>{i.elements(d)}</td>
                                            )
                                        ))
                                    }
                                </tr>
