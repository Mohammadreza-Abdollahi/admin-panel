...
const Input = ({name , label , secodeLabel = '' , type , value , placeholder , readOnly = false}) => {
    ...
    return ( 
        <div className={`flex ring-2 rounded-sm text-lg text-slate-800 overflow-hidden transition-all duration-150 ${focus ? 'ring-palete-4-500-1' : 'ring-palete-2-400-1'}`}>
            ...
            <input onFocus={handleFocus} onBlur={handleFocus} type={type} value={value === undefined ? null : value} name={name} readOnly={readOnly} placeholder={placeholder} className="appearance-none w-3/4 py-2 px-2 text-xl focus:outline-none"/>
            {
                secodeLabel !== '' ? (
