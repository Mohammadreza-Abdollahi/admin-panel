...
const ColorsTable = ({data , actionCol , rowInPage , searchable = false , dialogOpenner , searchParam = 'title' , hasBtn = true}) => {
                ...
                <table className="text-center w-full bg-palete-2-100 bg-opacity-60 rounded-sm overflow-hidden ring-1 ring-palete-2-300">
                    <thead className="border-b-palete-2-300 border-b-4 bg-palete-2-200 bg-opacity-70">
                        <tr className="text-slate-800">
                            ...
                            <th className="py-3">نام رنگ</th>
                            <th className="py-3">کد رنگ</th>
                            <th className="py-3">رنگ</th>
                            {
                                actionCol ? (
                    ...
                    <tbody>
                            {
                                tableData.map(d=>(
                                    <tr key={d.id} className="transition-all duration-150 border-b-palete-2-200 border-b-2 hover:bg-palete-2-300 hover:bg-opacity-50">
                                        ...
                                        <td className="py-2.5 ring-1 ring-palete-2-200">{d.colorName}</td>
                                        <td className="py-2.5 ring-1 ring-palete-2-200">{d.colorCode}</td>
                                        <td className="py-2.5 px-5 ring-1 ring-palete-2-200"><div style={{backgroundColor: `${d.colorCode}`}} className='w-full h-8 border-2 border-white outline outline-4 outline-palete-2-400-1 rounded-sm'></div></td>                                        
                                        {
                                            actionCol ? (
