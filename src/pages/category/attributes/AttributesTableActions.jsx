...
const AttributesTableActions = ({data , setAttributeToEdit , attributeToEdit}) => {
    return ( 
        <>
            <section className={`py-1.5 ${attributeToEdit && attributeToEdit.id === data.id ? "bg-palete-4-500-1 bg-opacity-30 rounded-sm " : ""}`}>
                <Tooltip arrow placement="top" onClick={()=>{setAttributeToEdit(data)}} title={<><span className="text-base">ویرایش دسته</span></>}><FontAwesomeIcon icon={faEdit} className="text-xl text-yellow-500 hover:bg-yellow-100 px-2 py-1 rounded-md cursor-pointer"/></Tooltip>
